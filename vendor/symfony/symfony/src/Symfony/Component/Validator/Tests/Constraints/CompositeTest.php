...
class CompositeTest extends \PHPUnit_Framework_TestCase
{
    public function testMergeNestedGroupsIfNoExplicitParentGroup()
    {
        $constraint = new ConcreteComposite(array(
            new NotNull(array('groups' => 'Demande')),
            new NotBlank(array('groups' => array('Demande', 'Strict'))),
        ));

        $this->assertEquals(array('Demande', 'Strict'), $constraint->groups);
        $this->assertEquals(array('Demande'), $constraint->constraints[0]->groups);
        $this->assertEquals(array('Demande', 'Strict'), $constraint->constraints[1]->groups);
    }
    ...
    public function testSetImplicitNestedGroupsIfExplicitParentGroup()
    {
        $constraint = new ConcreteComposite(array(
            ...
            'groups' => array('Demande', 'Strict'),
        ));

        $this->assertEquals(array('Demande', 'Strict'), $constraint->groups);
        $this->assertEquals(array('Demande', 'Strict'), $constraint->constraints[0]->groups);
        $this->assertEquals(array('Demande', 'Strict'), $constraint->constraints[1]->groups);
    }
    ...
    public function testFailIfExplicitNestedGroupsNotSubsetOfExplicitParentGroups()
    {
        new ConcreteComposite(array(
            'constraints' => array(
                new NotNull(array('groups' => array('Demande', 'Foobar'))),
            ),
            'groups' => array('Demande', 'Strict'),
        ));
    }
    ...
    public function testImplicitGroupNamesAreForwarded()
    {
        ...
        $this->assertEquals(array('Demande', 'Strict', 'ImplicitGroup'), $constraint->groups);
        $this->assertEquals(array('Demande', 'ImplicitGroup'), $constraint->constraints[0]->groups);
        $this->assertEquals(array('Strict'), $constraint->constraints[1]->groups);
    }
